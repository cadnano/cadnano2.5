...
from .nucleicacidpartitem import SolidNucleicAcidPartItem


class SolidRootItem(QEntity):
    """
    SolidRootItem is the root item in the SolidView. It gets added directly
    ...
    SolidRootItem must instantiate its own controller to receive signals
    from the model.
    ...
    def __init__(self, window, document):
        ...
        """
        super(SolidRootItem, self).__init__()
        self._window = window
    ...
    def partAddedSlot(self, sender, model_part_instance):
        ...
        if part_type == PartType.NUCLEICACIDPART:
            na_part_item = SolidNucleicAcidPartItem(model_part_instance,
